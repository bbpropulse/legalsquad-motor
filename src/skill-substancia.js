...
export function lerSubstanciaDoIndice(indice) {
  ...
  for (const linha of texto.split('\n')) {
    ...
    if (nome) {
      atual = nome[1];
      mapa.set(atual, { linhasProprias: undefined, originalidade: undefined });
      continue;
    }
    if (!atual) continue;
    ...
    if (originalidade) mapa.get(atual).originalidade = Number(originalidade[1]);
  }

  // Entradas sem nenhuma das duas medidas são ruído de parsing, não skills.
  for (const [id, valores] of mapa) {
    if (valores.linhasProprias === undefined && valores.originalidade === undefined) mapa.delete(id);
  }
  return mapa;
}
