...
After giving the session notice below and **before the first browser/web action**, reduce the research purpose to capability terms without case data and run `npx criminalsquad search-skills --query "<capability>" --limit 8 --json`. Use only that compact shortlist; never load `skills/_index.yaml` in full. For architecture work, `--include-preview` may expose test sources. Query `skills/_execucao-penal-v3-integration.yaml` only by targeted search when an `ep-*` bridge must be resolved.
...
If the squad purpose, reference profile or research topic involves **execução penal**, also read `_criminalsquad/core/best-practices/execucao-penal-alta-performance.md` before researching — **when the file exists**; if it does not (área não instalada), note `execution_penal_protocol: not_installed` in the `catalog_preflight` and continue, never reconstructing the protocol from memory. Apply its freshness, official-source, calculation, evidence, Citation Gate and human-review protocol. The `ep-*` preview files may inform an audit/evolution study, but the output must route production work to canonical released targets from the manifest.

Record a compact `catalog_preflight` note in `pattern-analysis.md`: files read, canonical capabilities already available, lifecycle exclusions, and (when applicable) that `execucao-penal-alta-performance` was applied.

## Session Management (ALWAYS inform the user)

At the START of every investigation, before any browser action, tell the user:

> "Your browser sessions are saved in `_criminalsquad/_browser_profile/`. To clear a platform's session, just delete the JSON file (e.g., `instagram.json`). I'll ask before saving any new session."
...
Browser sessions are stored as JSON files in `_criminalsquad/_browser_profile/`:

- Instagram: `_criminalsquad/_browser_profile/instagram.json`
- YouTube: `_criminalsquad/_browser_profile/youtube.json`
- Twitter/X: `_criminalsquad/_browser_profile/twitter.json`
- LinkedIn: `_criminalsquad/_browser_profile/linkedin.json`

**Loading a session (Playwright CLI):**
```bash
npx playwright open --load-storage=_criminalsquad/_browser_profile/{platform}.json {url}
```

**Saving a session (Playwright CLI):**
```bash
npx playwright open --save-storage=_criminalsquad/_browser_profile/{platform}.json {url}
```
...
Session files are stored in `_criminalsquad/_browser_profile/` as JSON files (one per platform). This directory is gitignored and persists between runs.
...
6. **Step 2 — Save the session (quick — user is already logged in):**
   ...
   If yes:
   ```bash
   npx playwright open --save-storage=_criminalsquad/_browser_profile/{platform}.json {platform-url}
   ```
...
7. If the user declines saving, inform them:
   "No problem. Remember your cookies are still in `_criminalsquad/_browser_profile/` — delete the folder to clear everything."
...
Users can clear all saved browser sessions at any time by deleting the `_criminalsquad/_browser_profile/` directory or individual JSON files. After deletion, the next investigation will require a fresh manual login.
