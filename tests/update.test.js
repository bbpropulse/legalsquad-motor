...
test('update preserves _memory contents', async () => {
  ...
  try {
    await init(tempDir, { _skipPrompts: true });
    await writeFile(
      join(tempDir, '_criminalsquad', '_memory', 'company.md'),
      'My Company Info',
      'utf-8'
    );
...
test('update preserves _investigations contents', async () => {
  ...
  try {
    await init(tempDir, { _skipPrompts: true });
    await writeFile(
      join(tempDir, '_criminalsquad', '_investigations', 'profile-analysis.md'),
      'investigation data',
      'utf-8'
    );
...
test('update writes new version to .criminalsquad-version', async () => {
  const tempDir = await mkdtemp(join(tmpdir(), 'criminalsquad-test-'));
  ...
  try {
    ...
    const version = await readFile(
      join(tempDir, '_criminalsquad', '.criminalsquad-version'),
      'utf-8'
    );
...
test('update succeeds without existing .criminalsquad-version (legacy install)', async () => {
  const tempDir = await mkdtemp(join(tmpdir(), 'criminalsquad-test-'));

  try {
    await init(tempDir, { _skipPrompts: true });
    await rm(join(tempDir, '_criminalsquad', '.criminalsquad-version'), { force: true });
...
test('update auto-imports bundled skills with env requirements', async () => {
  ...
  try {
    ...
    // Simulate a user who installed criminalsquad before this skill was bundled
