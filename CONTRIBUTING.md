...
- `templates/ide-assets/instructions-body.md` — instruções do projeto (CLAUDE.md/rules/etc.)

Editou um desses? Rode **`npm run build:ide`** para propagar para todos os arquivos
de `templates/ide-templates/` (o frontmatter de cada IDE é preservado). **Não edite o
corpo direto nos templates** — o teste `tests/ide-build.test.js` falha se eles saírem
de sincronia com os assets.

### Como cortar release (mantenedor)

Alunos instalam via `git+https` — **sem release cortada, eles recebem o HEAD de
`main` com a versão congelada** (o update anuncia "vX → vX" trocando dezenas de
arquivos). Ao acumular mudanças em `[Unreleased]` no CHANGELOG, corte a release:

```bash
npm run verify                     # tudo verde antes de cortar
