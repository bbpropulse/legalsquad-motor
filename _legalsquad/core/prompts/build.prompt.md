...
Se um especialista escolhido **não existe** em `.claude/agents/`, o defeito veio da Discovery (gravou um nome não instalado). Antes de descartar, **procure nome aproximado** entre os agentes realmente instalados (typo, plural, prefixo numérico): se houver correspondência óbvia, **corrija o nome** em vez de remover — perder reuso legítimo por erro de digitação é pior que o defeito original. Só quando não houver agente correspondente, remova-o de `specialist_agents`, ajuste o agente/step para cobrir o papel sem delegação fantasma e **apresente a remoção ao usuário junto com o Quality Report** — mesma cortesia do caso abaixo: quem escolheu o especialista foi ele, não você. Se existe mas **não** é referenciado por nenhum arquivo do squad, o reuso foi perdido. Corrija: faça o agente/step fino **delegar ao especialista pelo nome** (não recrie a expertise — ver Step B "Reused specialists"). Máx 2 tentativas; depois, apresente ao usuário (o especialista pode ser genuinamente dispensável — deixe o usuário confirmar antes de descartá-lo).
...
- [ ] `npx legalsquad audit-skills` sem hard fail estrutural e `npx legalsquad check-skills` íntegro

Se QUALQUER item falhar: ajuste o `SKILL.md`, rode `npx legalsquad contract-skills` de novo e revalide. Máx 2 tentativas; depois, apresente ao usuário a skill que não pôde ser garantida (nunca conclua o squad carregando uma skill nova fora do contrato).
