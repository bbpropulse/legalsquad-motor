...
function cmdInit(dir, flags) {
  ...
  if (typeof flags.run === 'string' && flags.run.trim()) {
    writeJson(dir, RUN_LEDGER, { ...abrirRun({ runId: flags.run, squad: readSquadCode(dir), total, agora: now() }), updatedAt: now() });
  }
  writeState(dir, {
...
function cmdStep(dir, flags) {
  ...
  s.updatedAt = now();
  writeState(dir, s);
  atualizarRunLedger(dir, (l) => avancarRun(l, { current, label: str(flags.label), agora: now() }));
}
...
function cmdReviewVerdict(dir, flags) {
  ...
  // Só gates de QUALIDADE (review/redacao/citacao): retry e veto medem
  ...
  if (result.action !== 'await' && ['revisao', 'redacao', 'citacao'].includes(gate)) {
    try {
